...
//	protected int profileCounter=0;
	...
	protected Profile createProfile(Profile profile)
	{
		try
		{
			return userService.registerBasicMember(profile);
		}
	...
	protected Profile createProfile(String email)
	{
		try
		{
			return userService.registerBasicMember(profile(email));
		}
		catch (ValidationException e)
		{
			Assert.fail(e.getMessage());
...
//	}
	...
	protected void createDatabaseDefaults()
	{
		...
		//load the roles
		roleUser = userDao.getRoleById(RoleEnum.HF_USER_ROLE.getId());
		roleAdmin = userDao.getRoleById(RoleEnum.HF_ADMIN_ROLE.getId());
		roleGuest = userDao.getRoleById(RoleEnum.GUEST.getId());
