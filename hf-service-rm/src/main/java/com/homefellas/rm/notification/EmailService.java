...
public class EmailService extends AbstractService implements IEmailService
{
	...
	{
		Profile userMakingChangeProfile = userService.getProfileByEmail(userMakingChange);
		Profile toEmailProfile = userService.getProfileByEmail(toEmail);
		
		if (userMakingChangeProfile == null || toEmailProfile == null)
			return;
		
		List<Notification> notifications = new ArrayList<Notification>();
		
		notifications.add(buildTaskStatusChangeNotification(taskBeingChanged, toEmailProfile, userMakingChangeProfile, NotificationTypeEnum.EMAIL, taskStatusChangeEvent));
		notifications.add(buildTaskStatusChangeNotification(taskBeingChanged, toEmailProfile, userMakingChangeProfile, NotificationTypeEnum.PUSH, taskStatusChangeEvent));
...
//		notification.setToProfileId(taskThatChanged.getTaskCreator().getId());
	...
	public void processNotificationsForTaskStatusChange(Task task, Task taskFromDB, String toEmail, String userChangingTaskEmail)
	{
		...
		if (taskFromDB.getStartTime()!=null && !taskFromDB.getStartTime().equals(task.getStartTime()))
			startDateChanged = true;
		
		if (taskFromDB.getEndTime()!=null && !taskFromDB.getEndTime().equals(task.getEndTime()))
			endDateChanged = true;
		...
		if (statusChanged)
		{
			...
			{
				sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.Completed);
//				emailService.sendNotificationForTaskCompleteOrDelete(taskFromDB, NotificationTypeEnum.EMAIL, task.getTaskCreator().getMember().getEmail());	
			...
			{
				sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.InProgress);
			}
			else if (task.getProgress() == ProgressEnum.ON_HOLD.ordinal())
			{
				sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.Waiting);
			}
			else if (task.getProgress() == ProgressEnum.DELETE.ordinal())
			{
				sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.Canceled);
			}
			
		}
		else if (startDateChanged)
		{
			sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.StartDate);
		}
		else if (endDateChanged)
		{
			sendTaskStatusChangeNotification(task, userChangingTaskEmail, toEmail, TaskStatusChangeEvent.EndDate);
		}	
	}
	...
	{
		createAndCancelTaskEndTimeNotifications(task, taskFromDB, task.getTaskCreator().getEmail(), isUpdate);
	}
	...
	public void createAndCancelTaskEndTimeNotifications(Task task, Task taskFromDB, String toEmailAddress, boolean isUpdate)
	{
		...
		if (task.getEndTime()!=null && task.getEndTime().isAfter(new DateTime()))
		{
			...
			if (isUpdate)
			{
				Notification notification = notificationService.getNotificationBySendToAndDateTime(toEmailAddress, taskFromDB.getEndTime());
...
//		String email;
		...
		{
			Profile profile = userService.loadProfileById(task.getTaskCreator().getId());
...
//		}
	...
	public Notification sendShareCalendarAcceptOrDeclineNotificationToTaskOwner(ShareCalendar shareCalendar, Profile profile, NotificationTypeEnum notificationTypeEnum, ShareApprovedStatus shareApprovedStatus)
	{
		...
		return sendShareAcceptOrDeclineNotificationToTaskOwner(shareCalendar, shareCalendar.getCalendar().getMember().getMember(), shareCalendar.getCalendar().getCalendarName(), name, notificationTypeEnum, shareApprovedStatus);
	}
