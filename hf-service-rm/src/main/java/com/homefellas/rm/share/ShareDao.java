...
public class ShareDao extends HibernateCRUDDao implements IShareDao
{
...
//	}
	...
	{
		Query query = getQuery("from ShareCalendar s where s.id in (:ids)");
		query.setParameterList("ids", ids);
...
//		Query query = getQuery("select p from Share share inner join fetch Profile p on share.user.id = p.id where share.task.taskCreator.id = ?");
	...
	public Invite getInviteById(String id) {
		return loadByPrimaryKey(Invite.class, id);
	}
	...
	public ShareCalendar getShareCalendarById(String id) {
		return loadByPrimaryKey(ShareCalendar.class, id);
	}

	@Override
	public ShareCalendar updateShareCalendar(ShareCalendar shareCalendar) {
		updateObject(shareCalendar);
