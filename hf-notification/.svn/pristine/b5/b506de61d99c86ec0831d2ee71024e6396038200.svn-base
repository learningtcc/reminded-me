...
System.out.println("Sending sms...");
	...
	public boolean sendEmail(final Notification notification) throws NotificationLauncherException
	{
		...
		try
		{
			fromAddress = InternetAddress.parse(notification.getSendFrom())[0];
			toAddress = InternetAddress.parse(notification.getSendTo())[0]; 
			
			if (notification.getSendFromAlias()==null || notification.getReplyTo() == null)
				fromAddress.setPersonal(notification.getSendFrom());
			else
				fromAddress.setPersonal(notification.getSendFromAlias()+" as "+notification.getReplyTo());
